...
class MovieTests {
  ...
  private final Path filteredMovies = Path.of(
      "src/integrationTest/resources/fixtures/filtered-movies.json");
  ...
  void shouldReturnFilteredMovies() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/movie/filtered").param("filter", TestDataFactory.FIRST_FILTER_UUID.toString()))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
        .andExpect(content().json(expectedMovies));
  }
  ...
  void shouldReturnErrorFilterNotFound() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/movie/filtered").param("filter", "f4eb74bd-9e6d-409b-9bef-6928d7de25e3"))
        .andExpect(MockMvcResultMatchers.status().isNotFound())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
        .andExpect(content().json(
            "{\"message\":\"Filter f4eb74bd-9e6d-409b-9bef-6928d7de25e3 not found\"," +
